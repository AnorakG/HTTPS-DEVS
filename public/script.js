...
function confirmarIdade() {
  ...
  idade = Math.floor(
    (hoje - dataDeNascimento) / (365.25 * 24 * 60 * 60 * 1000)
  );
  console.log(idade);
}
...
function cadastrar() {
  ...
  if (
    senha1.value.length == 0 ||
    senha2.value.length == 0 ||
    nome.value.length == 0 ||
    email.value.length == 0 ||
    dataNascimento.value.length == 0
  ) {
    Swal.fire({
...
function logOut() {
  ...
  for (var i = 0; i < cookies.length; i++) {
    ...
    document.cookie =
      name + "=;expires=Thu, 01 Jan 1970 00:00:00 GMT;path=./sa;";
  }
  Swal.fire({
...
function perfil() {
  ...
  let dataDeNascimentoUsuarioPerfil = document.getElementById(
    "dataNascimentoUsuarioPerfil"
  );
  ...
  dataDeNascimentoUsuarioPerfil.innerHTML = perfilConectado.dataDeNascimento
    .split("-")
    .reverse()
    .join("/");
